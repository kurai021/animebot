...
class Wikia {
  constructor({ wiki } = {}) {
     ...
     * @type {String}
     */
    this.baseURL = `http://www.${wiki ? `${wiki}.` : ""}fandom.com/es/api/v1`;
  ...
  */
   ...
   * @param {String|Array<String>} [options.thumbnail] - The absolute URL of the thumbnail.
   * @param {String|Array<String>} [options.url] - The relative URL of the Article. Absolute URL: obtained from combining relative URL with basepath attribute from response.
   ...
   */
  getArticleDetails(ids, { titles, abstract, width, height, thumbnail, url } = {}) {
    if(Array.isArray(ids)) ids = ids.join(",");
    ...
    if(url) req.body({ url })
    if(thumbnail) req.query({ thumbnail })
