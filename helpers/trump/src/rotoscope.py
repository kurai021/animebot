...
fonts="{0}/fonts".format(os.path.dirname(__file__))

# Fonts
impact = ImageFont.truetype('{0}/impact.ttf'.format(fonts), 38)
textFont = None
...
def computeAndLoadTextFontForSize(drawer, text, maxWidth):
	...
	while curSize >= minSize:
		textFont = ImageFont.truetype('{0}/impact.ttf'.format(fonts), size=curSize)
...
def generateText(text):
	...
	newline = 0
	for word in words:
		draw.text((xCenter, 10 + yCenter + newline), word, font=impact, fill=txtColor)
		newline += 40
